public class IntersectionofTwoLinkedLists {
	 ...
	 * 3 nodes before the intersected node in B. - Note that the intersected node's
	 * value is not 1 because the nodes with value 1 in A and B (2nd node in A and
	 * 3rd node in B) are different node references. In other words, they point to
	 * two different locations in memory, while the nodes with value 8 in A and B
	 ...
	 * the n. 1 <= m, n <= 3 * 104 1 <= Node.val <= 105 0 <= skipA <= m 0 <= skipB
    ...
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
    	...
    	if(diff>0) {   //L1 is greater than L2
    		while(diff>0) {
    			ptr1 = ptr1.next;
    			diff--;
    		}
    	}else {     //L2 is greater than L1
    		while(diff<0) {
    			ptr2 = ptr2.next;
    			diff++;
    		}
